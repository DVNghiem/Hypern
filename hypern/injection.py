...
class HandlerPlan:
    ...
    async def invoke(
        ...
        arguments = [
            await parameter.resolver(req, res, ctx, scope)
            for parameter in self.positional_parameters
        ]
        if self.keyword_parameters:
            keyword_arguments = {
                cast(str, parameter.keyword_name): await parameter.resolver(req, res, ctx, scope)
...
def _transport_resolver(name: str) -> _HandlerResolver:
    async def resolve(req: object, res: object, ctx: object, scope: RequestScope) -> object:
        if name == "req":
            return req
        if name == "res":
            return res
