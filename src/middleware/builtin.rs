//! Built-in middleware implementations in pure Rust.
//! 
//! These middleware provide common functionality without any Python/GIL overhead:
//! - CORS handling
//! - Rate limiting
//! - Request logging
...
use super::chain::{
    HttpMethod, MiddlewareContext, MiddlewareResponse, MiddlewareResult,
...
// ============================================================================
// Compression Middleware (marks for compression)
// ============================================================================
...
// ============================================================================
// Path-based Middleware (runs only on specific paths)
// ============================================================================
...
// ============================================================================
// Method-based Middleware (runs only on specific methods)
// ============================================================================
...
mod tests {
    ...
    async fn test_cors_preflight() {
        ...
        let ctx = MiddlewareContext::new("/api", HttpMethod::OPTIONS, headers, "", None);
        let result = cors.execute(&ctx).await;
        
        assert!(matches!(result, MiddlewareResult::Response(r) if r.status == 204));
    }

    #[tokio::test]
    async fn test_rate_limit() {
        // Use fixed window for deterministic testing
        ...
        let ctx = MiddlewareContext::new("/api", HttpMethod::GET, HashMap::new(), "", None);
        
        // First request should pass
        assert!(matches!(limiter.execute(&ctx).await, MiddlewareResult::Continue));
        
        // Second request should pass
        assert!(matches!(limiter.execute(&ctx).await, MiddlewareResult::Continue));
        
        // Third request should be rate limited
        assert!(matches!(limiter.execute(&ctx).await, MiddlewareResult::Response(r) if r.status == 429));
    }

    #[tokio::test]
    async fn test_security_headers() {
        let security = SecurityHeadersMiddleware::default_headers();
        let ctx = MiddlewareContext::new("/", HttpMethod::GET, HashMap::new(), "", None);
        
        let result = security.execute(&ctx).await;
        assert!(matches!(result, MiddlewareResult::Continue));
        
        let headers = ctx.response_headers.read();
    ...
    async fn test_basic_auth_success() {
        let auth = BasicAuthMiddleware::new("Test Realm")
            .add_user("admin", "secret");
        
        let mut headers = HashMap::new();
        // "admin:secret" in base64 = "YWRtaW46c2VjcmV0"
        headers.insert("authorization".to_string(), "Basic YWRtaW46c2VjcmV0".to_string());
        
        let ctx = MiddlewareContext::new("/", HttpMethod::GET, headers, "", None);
        let result = auth.execute(&ctx).await;
    ...
    async fn test_basic_auth_failure() {
        ...
        assert!(matches!(result, MiddlewareResult::Response(r) if r.status == 401));
    }
}
