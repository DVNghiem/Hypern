//! Pure Rust middleware chain implementation following ExpressJS architecture.
//! 
//! This module provides high-performance middleware execution without any Python/GIL overhead.
//! Middleware can modify requests, short-circuit responses, or pass control to the next handler.
...
pub struct RustMiddlewareChain {
...
impl Default for RustMiddlewareChain {
    fn default() -> Self {
        Self::new()
    }
}

impl Clone for RustMiddlewareChain {
    fn clone(&self) -> Self {
        Self {
...
impl RustMiddlewareChain {
    ...
    pub async fn execute_error(&self, ctx: &MiddlewareContext, error: &MiddlewareError) -> Option<MiddlewareResponse> {
        ...
        ctx.set_state("error_code", StateValue::String(error.code.clone()));
        ctx.set_state("error_message", StateValue::String(error.message.clone()));
        ctx.set_state("error_status", StateValue::Int(error.status as i64));
...
impl MiddlewareChainBuilder {
    ...
    /// Build the chain
    pub fn build(self) -> RustMiddlewareChain {
        self.chain
    }
}
...
mod tests {
    ...
    impl RustMiddleware for TestMiddleware {
        fn name(&self) -> &'static str {
            self.name
        }
        ...
        fn execute<'a>(
            ...
            Box::pin(async move {
                ctx.set_state(self.name, StateValue::Bool(true));
                if should_continue {
                    MiddlewareResult::Continue
                } else {
                    MiddlewareResult::Response(
                        MiddlewareResponse::new(403).with_text_body("Blocked")
                    )
                }
            })
        }
    ...
    async fn test_middleware_chain_continue() {
        let mut chain = RustMiddlewareChain::new();
        chain.use_before(TestMiddleware { name: "test1", should_continue: true });
        chain.use_before(TestMiddleware { name: "test2", should_continue: true });

        let ctx = MiddlewareContext::new("/test", HttpMethod::GET, HashMap::new(), "", None);
        let result = chain.execute_before(&ctx).await;
        
        assert!(matches!(result, MiddlewareResult::Continue));
        assert!(ctx.get_state("test1").is_some());
        assert!(ctx.get_state("test2").is_some());
    }

    #[tokio::test]
    async fn test_middleware_chain_short_circuit() {
        let mut chain = RustMiddlewareChain::new();
        chain.use_before(TestMiddleware { name: "test1", should_continue: true });
        chain.use_before(TestMiddleware { name: "blocker", should_continue: false });
        chain.use_before(TestMiddleware { name: "test2", should_continue: true });
        ...
        assert!(ctx.get_state("test1").is_some());
        assert!(ctx.get_state("blocker").is_some());
        // test2 should not have run
        assert!(ctx.get_state("test2").is_none());
    }
}
