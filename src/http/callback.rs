...
impl PyFutureAwaitable {
    ...
    pub(crate) fn set_result(pyself: Py<Self>, py: Python, result: Py<PyAny>) {
        let rself = pyself.get();

        _ = rself.result.set(Ok(result));
        if rself
            .state
            .compare_exchange(
        ...
        {
            let ack = rself.ack.read();
            if let Some((cb, ctx)) = ack.as_ref() {
