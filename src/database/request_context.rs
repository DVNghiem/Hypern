...
use super::pool::{ConnectionPoolManager, get_db_runtime};
use super::row_converter::{RowConverter, DynParam};
...
static REQUEST_CONTEXTS: OnceLock<DashMap<String, HashMap<String, Arc<DatabaseContextInner>>>> = OnceLock::new();
...
impl DbSession {
    ...
    fn query(&self, py: Python<'_>, sql: &str, params: Option<Vec<Py<PyAny>>>) -> PyResult<Vec<Py<PyAny>>> {
        let ctx = self.context.clone();
        ...
        let rows = get_db_runtime().block_on(async move {
            ctx.query(&sql, &converted_params).await
        }).map_err(|e| PyRuntimeError::new_err(e))?;
    ...
    #[pyo3(signature = (sql, params=None))]
    fn query_one(&self, py: Python<'_>, sql: &str, params: Option<Vec<Py<PyAny>>>) -> PyResult<Py<PyAny>> {
        let ctx = self.context.clone();
        let sql = sql.to_string();
        let params = params.unwrap_or_default();
    ...
    fn execute(&self, py: Python<'_>, sql: &str, params: Option<Vec<Py<PyAny>>>) -> PyResult<u64> {
        ...
        get_db_runtime().block_on(async move {
            ctx.execute(&sql, &converted_params).await
        }).map_err(|e| PyRuntimeError::new_err(e))
    }

    #[pyo3(signature = (sql, params_list))]
    fn execute_many(&self, py: Python<'_>, sql: &str, params_list: Vec<Vec<Py<PyAny>>>) -> PyResult<u64> {
        let ctx = self.context.clone();
        let sql = sql.to_string();
        ...
        for params in params_list {
            ...
            let affected = get_db_runtime().block_on(async move {
                ctx_clone.execute(&sql_clone, &converted_params).await
            }).map_err(|e| PyRuntimeError::new_err(e))?;
    ...
    fn __repr__(&self) -> String {
        format!("DbSession(request_id='{}', alias='{}')", self.context.request_id(), self.context.alias())
    }
}

pub fn create_request_context(request_id: &str, alias: &str) -> Arc<DatabaseContextInner> {
    let ctx = Arc::new(DatabaseContextInner::new(request_id.to_string(), alias.to_string()));
    
    let contexts = get_contexts();
    let mut request_contexts = contexts.entry(request_id.to_string()).or_insert_with(HashMap::new);
...
pub async fn finalize_all_request_contexts(request_id: &str) -> Result<(), String> {
    ...
    let contexts_opt = {
        get_contexts().get(request_id).map(|r| r.clone())
    };
...
pub fn finalize_db(request_id: &str, alias: &str) -> PyResult<()> {
    ...
    get_db_runtime().block_on(async move {
        finalize_request_context(&request_id, &alias).await
    }).map_err(|e| PyRuntimeError::new_err(e))
}
...
pub fn finalize_db_all(request_id: &str) -> PyResult<()> {
    let request_id = request_id.to_string();
    get_db_runtime().block_on(async move {
        finalize_all_request_contexts(&request_id).await
    }).map_err(|e| PyRuntimeError::new_err(e))
}
