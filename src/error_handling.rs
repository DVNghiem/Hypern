...
impl HypernError {
    ...
    fn __str__(&self) -> String {
        format!("{}: {} ({})", self.error_type.error_name(), self.message, self.status_code)
    }

    fn __repr__(&self) -> String {
        format!("HypernError('{}', {}, '{}')", self.message, self.status_code, self.error_type.error_name())
    }
}

impl fmt::Display for HypernError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}: {} ({})", self.error_type.error_name(), self.message, self.status_code)
    }
}

impl std::error::Error for HypernError {}

/// Error handler trait for middleware
pub trait ErrorHandler {
    fn handle_error(&self, error: &HypernError) -> PyResult<PyObject>;
}
...
macro_rules! hypern_error {
    ($error_type:ident, $message:expr) => {
        HypernError::new($message.to_string(), None, Some(stringify!($error_type).to_string()))
    };
    ($error_type:ident, $message:expr, $status:expr) => {
        HypernError::new($message.to_string(), Some($status), Some(stringify!($error_type).to_string()))
    };
}

// Convenience functions for common errors
pub fn bad_request(message: &str) -> HypernError {
    HypernError::new(message.to_string(), Some(400), Some("BadRequest".to_string()))
}

pub fn unauthorized(message: &str) -> HypernError {
    HypernError::new(message.to_string(), Some(401), Some("Unauthorized".to_string()))
}

pub fn forbidden(message: &str) -> HypernError {
    HypernError::new(message.to_string(), Some(403), Some("Forbidden".to_string()))
}
...
pub fn internal_server_error(message: &str) -> HypernError {
    HypernError::new(message.to_string(), Some(500), Some("InternalServerError".to_string()))
}