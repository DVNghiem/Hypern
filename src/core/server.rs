...
use crate::middleware::{
    HttpMethod as MwHttpMethod, MiddlewareContext, MiddlewareResponse, MiddlewareResult,
    RustMiddlewareChain,
};
...
pub struct Server {
    ...
    rust_middleware: Arc<RustMiddlewareChain>,
}
...
impl Server {
    ...
    pub fn new() -> Self {
        Self {
            ...
            rust_middleware: Arc::new(RustMiddlewareChain::new()),
        }
    }
                            ...
                            .serve_connection(
                                ...
                                service_fn(move |req| {
                                    ...
                                    async move {
                                        ...
                                        let method = MwHttpMethod::from_str(fast_req.method().as_str())
                                            .unwrap_or(MwHttpMethod::GET);
                                        ...
                                        if let Some((route, params)) = router.find_matching_route(
                                            ...
                                            // Apply any response headers added by middleware
                                            // Note: The response is already built by Python handler,
                                            // so middleware headers are applied there
...
impl Server {
    ...
    pub fn set_rust_middleware_chain(&mut self, chain: RustMiddlewareChain) {
